...
在没有选中任何文字的情况下，按下按钮会在光标两边插入标签。

在帖子编辑器内选中文本时，单击按钮将在文本两边插入标签。
...
从读者的观点中**隐藏一个段落**。然而，读者仍然可以通过点击剧透框来阅读这段文字，来查看隐藏的段落。
...
从读者的角度看，把一个段落隐藏在一个可折叠的盒子里。然而，读者仍然可以通过点击折叠框来显示隐藏的段落以阅读这段文字。
...
使用 monospaced 字体格式化文本，并将其放在一个灰色的盒子里。当您为一个项目发布故事版代码，或发布源代码时，这是非常有用的。
...
用所提供的文本，使用 Google 的 *我感到幸运* 按钮，**添加一个指向该页面的直链**。

- 专用按钮： ![幸运](img/lucky.jpg "幸运")
- 注：
  - 要知道，这不会给每个人带来相同的结果。
    - 一些搜索结果由于语言或位置的原因而被隐藏。
- 语法：
